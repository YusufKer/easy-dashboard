...
interface FlavourCardProps {
  ...
  onUpdate: (flavourId: number, newPrice: string) => Promise<void>;
  onDelete: (flavourId: number) => Promise<void>;
  isDeleting: boolean;
  isUpdating: boolean;
}
...
export default function FlavourCard({
  ...
  const [newPrice, setNewPrice] = useState(flavour.price);

  const handleSave = async () => {
    await onUpdate(flavour.id, newPrice);
    setIsEditing(false);
  };

  const handleCancel = () => {
    setIsEditing(false);
    setNewPrice(flavour.price);
  };
